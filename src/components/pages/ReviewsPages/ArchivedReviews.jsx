  ...
  const postReview = async (rowData) => {
    ...
    if (response.status === "1") {
      ...
      toast.current.show({
          ...
          detail: response.responsemessage || "You have successfully posted Review "
        });
      } else {
          throw new Error(
            response
...
return (
    ...
    {loading && (
      ...
      >
        ...
        <Column field="reviewType" header="Review Type" body={checkboxTemplate} style={{ width: "15%" }} />
        <Column field="selectDoctor" header="Select Doctor" body={selectDoctorTemplate} />
        <Column field="action" header="Action" body={actionBodyTemplate} style={{ width: "10%", padding: "0px 5px" }} />
      </DataTable>
    </div>
)
}
